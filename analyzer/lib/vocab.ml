...
(* print progress bar *)
let prerr_progressbar silent ?(itv=1) : int -> int -> unit
= fun n total ->
  if silent || !Options.opt_nobar then () else
...
let my_prerr_endline silent str = if not silent then prerr_endline str
let my_prerr_newline silent () = if not silent then prerr_newline ()
let my_prerr_string silent str = if not silent then prerr_string str

let prerr_memory_usage ?(silent=false) () =
  if silent then ()
  else
    let stat = Gc.stat () in
